...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        log("Test Goal:" + std::to_string(goal.x) + " ," + std::to_string(goal.y));
        ...
        aStar_search(graph, map, Coordinates{x, y}, testGoal, came_from_aStar, cost_so_far_aStar, path_aStar);
        log("End Time = " + currentDateTime());
        followpath(map, path_aStar);
        // empty the datastructures for next round
